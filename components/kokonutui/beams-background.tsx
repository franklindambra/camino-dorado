...
function createBeam(width: number, height: number, isDarkMode: boolean): Beam {
  ...
  const hueBase = 20; // orange
  const hueRange = 4; // tight variation
...
export default function BeamsBackground({
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      beam.hue = hueBase + Math.random() * hueRange;
  ...
  return (
    <div
      className={cn(
        "relative min-h-full w-full overflow-hidden bg-neutral-100 dark:bg-neutral-950",
        className
      )}
    >
      <canvas
